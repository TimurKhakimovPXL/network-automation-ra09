...
def handle(device_params: dict, device_name: str, change: dict) -> dict:
    ...
    if response.status_code == 404:
        if iface_type in xml.VIRTUAL_INTERFACE_TAGS:
            # Virtual interface not yet created in running-config; a
            # non-existent interface is logically admin-down. If desired
            # is "up" the comparison detects a delta and the write below
            # creates and unshuts the interface; if desired is "down"
            # the comparison matches and the write is skipped.
            current_state = "down"
        else:
