...
def handle(device_params: dict, device_name: str, change: dict) -> dict:
    ...
    if response.status_code == 404:
        if iface_type in xml.VIRTUAL_INTERFACE_TAGS:
            ...
            # edit-config write below will create it. Mirror the shape
            # of _extract_switchport's empty return so _states_match
            # detects a delta iff the change actually declares a mode.
            current = {
                "mode":          None,
                "access_vlan":   None,
                "native_vlan":   None,
